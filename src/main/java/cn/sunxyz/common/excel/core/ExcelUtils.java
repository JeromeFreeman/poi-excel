...
public class ExcelUtils<T> extends AbstractExcelUtilss<T>{
	...
	public void createHeader(HSSFWorkbook wb, HSSFSheet sheet) {
		// 产生一行
		HSSFRow row = ExcelTool.createRow(wb,sheet);
...
//		style.setFillBackgroundColor(HSSFColor.GREY_40_PERCENT.index);
	...
	public void createRow(HSSFWorkbook wb,Object t, HSSFSheet sheet) {
		HSSFCell cell;// 产生单元格
		if (flag) {
			row = ExcelTool.createRow(wb,sheet);
		}
					...
					case MAP:
						...
						if (map != null) {
							...
							try {
								...
								if (ea.isExport()) {
									cell = ExcelTool.createCell(row,getExcelCol(ea.column()));// 创建cell
...
//		logger.debug(rowStart+"====>"+rowEnd);
		for (Field field : fields) {
			if(field.isAnnotationPresent(ExcelAttribute.class)){
				...
				if(!field.isAccessible()){
					field.setAccessible(true);//取消访问检查，提高速度
				}
...
//						logger.debug(key+" ===>"+value);  
						...
						String value = ExcelTool.getCellValue(fCell);
						tuples.put(key, value);
					}
				}
			}
